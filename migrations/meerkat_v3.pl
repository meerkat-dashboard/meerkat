...
# Perform migration
perl -MData::Dumper -MJSON -n0777 -le'$j = decode_json($_); %type_replace = qw(iframe-video video audio-stream audio static-image image); %opt_replace = qw(statusFontSize fontSize id objectName); foreach my $e (@{$j->{elements}}) {foreach my $r (keys %type_replace) {$e->{type} = $type_replace{$r} if $e->{type} eq $r}; foreach my $r (keys %opt_replace) {$e->{options}->{$opt_replace{$r}} = delete $e->{options}->{$r} if exists $e->{options}->{$r}}; delete $e->{options}->{StrokeColor}} print encode_json($j)' < $backup_name | jq -S . > $dashboard_name; done